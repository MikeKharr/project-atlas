...
- [ ] **T13. `lib/vocab/en.js` and `language`.** Add the `en` module with the values of spec §8, key for key with `ru.js` except the optional `placeholder`, which `en` does not set (§8); `lib/config.js` accepts `language` (`ru` | `en`, absent → `ru`, anything else → class-1 finding) and hands the selected module to the modules that take a vocabulary. Test fixture `test/fixtures/minimal-en/` — the English twin of the minimal fixture (same files and graph, English headings, statuses, labels and one gate trace with an English mark and one cancelled by a negation) with `"language": "en"` in its config.
  - Acceptance: `node build.js --root test/fixtures/minimal-en --check` exits 0 and its graph has the same node and edge counts as the `ru` fixture; a config with `"language": "de"` is a finding; under `en` a citation whose name merely contains the word (`` `guides/name-format.md` ``, ADR `` `2026-01-01-0000-name-service.md` ``) is still a citation; T7 stays green with `language` absent and with `"language": "ru"` added to the example config (byte equality unchanged); no module holds a Russian literal used for **matching input** — `grep -rn "Статус\|Владеет\|Заменяет\|вето" lib/` shows the vocabulary modules themselves (`lib/vocab/ru.js`) and nothing else except the fixed Russian text of vault notes, which is **output**, not matching: today exactly one line, the role-fact label `` `- Владеет:` `` in `lib/vault.js`. Those texts stay Russian under any `language` (spec §8, "Independent of `language`…") and that line is part of the byte-for-byte vault comparison of the mandatory criterion, so it must not change. `test/vocab.test.js` allows that one line by an explicit list with the reason, asserts the allowance still matches exactly one line, and fails on any new occurrence.
