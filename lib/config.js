...
import en from './vocab/en.js'
import ru from './vocab/ru.js'

/**
 * Встроенные словари соглашений документов (§8). Ключ `language` выбирает
 * один; без ключа — `ru`, поэтому пример ai-advent остаётся как был.
 */
const VOCABS = { ru, en }
...
export function validateConfig(raw, { file, text: source = '' }) {
  ...
  if (raw.format === 1) {
    // Формат 1 называется отдельно: у него закрытый список правок, и читатель
    // должен увидеть их все, а не гадать, что изменилось.
    bad(
      'format',
      'поддерживается только формат 2, задан 1: узлы `day` стали `unit`, в overlay `about.*.days` → `about.*.units`, поднимите `format` до 2',
    )
  } else if (raw.format !== 2) {
    bad('format', `поддерживается только формат 2, задан ${JSON.stringify(raw.format)}`)
  }

  if (raw.language !== undefined && VOCABS[raw.language] === undefined) {
    const known = Object.keys(VOCABS)
      .map((k) => `\`${k}\``)
      .join(' или ')
    bad('language', `встроенного словаря \`${raw.language}\` нет; в формате 2 — ${known}`)
  }
...
function resolve(raw, file) {
  ...
  return {
    ...
    /** Словарь соглашений: его получают markdown, extract, fired и vault. */
    vocab: VOCABS[raw.language ?? 'ru'],
    docs: {
      root: d.root,
