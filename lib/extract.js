...
// цитирования. Порт `atlas/lib/extract.js` (ai-advent-2026 на 1d882f4):
...
export function buildGraph(sources, config) {
  ...
  const unitId = (name) => (isUnit(name) ? `day/${name}` : `service/${name}`)
  for (const s of compose.services) {
  ...
  for (const entry of sources.adr) {
    ...
    for (const other of replaces) {
      ...
      else note(entry.path, 1, `в статусе «${vocab.replaces}» указан ADR \`${other}\`, которого нет`)
    }
  ...
  for (const { id, entry } of citing) {
    ...
    const days = override ? override.days : unitWord ? [...new Set([...entry.key.matchAll(unitWord)].map((m) => m[1]))] : []
    for (const day of days) {
      if (has(`day/${day}`)) link(id, `day/${day}`, 'about')
      else note(overlayFile, lineOf(sources.overlayText, `"${day}"`), `в about указан день \`${day}\`, которого нет в ${unitsWhere}`)
    }
  }
