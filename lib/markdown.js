...
export function makeGrammar(config) {
  const { docs, vocab } = config
  // Заглушки шаблонов и таблиц — не цитаты: `adr/YYYY-MM-DD-HHMM-title.md`,
  // `development-history/id`, `guides/name.md`. Слово-заглушка словаря —
  // ветка необязательная: без неё остаётся литерал `name.md`, и цитата, в
  // имени которой это слово просто встречается, цитатой быть не перестаёт.
  const PLACEHOLDER = new RegExp(
    ['YYYY', 'HHMM', '<', '\\*', vocab.placeholder ? `\\b${vocab.placeholder}\\b` : null, 'name\\.md', '\\/id$']
...
/**
 * Строки замены — только в разделе статуса, иначе в выборку попадают
 * таблицы и прочая проза. Слова обеих строк и имя раздела — из словаря.
 ...
 */
export function replacementRefs(text, vocab) {
