...
export const VAULT_DIRS = ['adr', 'history', 'design', 'guides', 'invariants', 'roles', 'units', 'services', 'skills', 'classes', 'phases']
...
export function buildVault({ graph, sources, provenance, config }) {
  ...
  const unitOf = (id) => {
    const bound = out(id, 'about').map((e) => byId.get(e.to)?.key)
    return bound.length > 0 ? unitNumber(config, bound[0]) : null
  }
  ...
  for (const node of graph.nodes) {
    ...
    note(
      noteOf.get(node.id),
      {
        ...
        unit,
        tags: [`type/${node.type}`, node.status ? statusTag(node.status, config.vocab) : null, unit ? `unit/${unit}` : null].filter(Boolean),
      },
      node.file,
  ...
  for (const node of graph.nodes.filter((n) => n.type === 'unit')) {
    ...
    note(
      noteOf.get(node.id),
      { type: 'unit', id: node.key, title: node.title, date: node.date, unit: n, tags: ['type/unit', `unit/${n}`] },
