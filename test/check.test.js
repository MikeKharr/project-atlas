...
test('строка замены на несуществующий ADR — находка', () => {
  ...
  // Сообщение не повторяет слова словаря: они у каждого языка свои.
  assert.ok(
    found.some((f) => f.file === ADR && /в строке замены статуса указан ADR `2025-12-31-2359`/.test(f.message)),
